...
const OBT_MNEMONIC_WORD_TO_INDEX: Record<string, number> = OBT_MNEMONIC_WORDLIST.reduce(
  (acc, word, index) => {
    acc[word] = index;
    return acc;
  },
...
type ObtMnemonicWords = [string, string, string];

function obtMnemonicWordlist(): string[] {
  return [...OBT_MNEMONIC_WORDLIST];
}

function obtMnemonicEncodeByte(byteValue: number): ObtMnemonicWords {
...
function obtMnemonicEncode(byteValue: number): string {
  return obtMnemonicEncodeByte(byteValue).join(" ");
}

function obtMnemonicDecode(mnemonic: string): number {
  ...
  if (checksum !== expectedChecksum) {
    throw new Error(
      `Checksum mismatch: '${words[2]}' (${checksum}) != expected ${expectedChecksum} ` +
...
function obtMnemonicValidate(mnemonic: string): boolean {
  try {
